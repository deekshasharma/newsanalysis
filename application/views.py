...
def get_stopwords():
	...
	if stopwords:
		...
		for words in open('/Users/deeksha/work/projects/crawler/newsanalysis/application/static/english','r').readlines():
...
def get_news(seedurl):
	...
	for eachheadline in get_headlines_from_seedurl(seedurl):
		...
		for data in soup.findAll('p',class_="body"):
			try:
				wordlist = re.split('\W',(unicodedata.normalize('NFKD', data.find(text=True)).encode('ascii','ignore')).lower())
...
def retrieve_from_database(fordate):
	return database_connection().find_one(fordate)
...
def what_data():
	...
	return render_template('citynewssummary.html',data=data)
